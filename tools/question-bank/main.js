...
const ANALYSIS_STYLE_MAP = {
    default:  '以清晰、准确为原则撰写解析',
    simple:   '用通俗易懂的语言撰写解析，避免专业术语，适合初学者理解',
    detailed: '详细深入地撰写解析，展开相关知识点的背景和延伸，帮助深入理解',
    brief:    '简洁精炼地撰写解析，只点明核心要点，不展开',
    example:  '结合具体实例撰写解析，通过实际案例帮助理解知识点',
};
...
function buildPrompt() {
    ...
    const countRule = aiMode === 'single'
        ...
        : '根据用户提供的知识内容生成题目，数量以完整覆盖所有知识点为准，不限上限，不遗漏任何知识点';
    const chapterRule = chapter ? `Chapter 字段统一填写「${chapter}」` : 'Chapter 字段根据内容自行归类';

    const base = `你是一个专业的题目出题助手。根据用户提供的知识内容生成题目。
...
- 第一行是表头，后续每行一道题
- 字段含逗号或换行时必须用英文双引号包裹，字段内部的双引号用两个双引号转义
- 必须完整保留所有题目，数量以完整覆盖所有知识点为准，不得遗漏任何一道
...
字段要求：
- Type：固定填写 cloze
- ClozeText：在关键位置用 [[正确答案]] 挖空
- 每道题可挖 1~3 个空，挖空内容应是关键术语或核心概念
