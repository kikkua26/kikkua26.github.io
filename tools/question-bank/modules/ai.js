...
- JSON 的键和字符串值必须使用半角双引号，字符串值内部若出现半角双引号，必须替换为全角双引号""或使用反斜杠转义：\\"
- ${countRule}
- ${chapterRule}
...
- **删除**：背景介绍、历史渊源、冗长的修辞、重复的解释、次要的修饰词、过渡句
- **压缩**：长段落改为短句，删除"我们可以看到""需要注意的是"等废话
- 每道题的解析控制在 30~60 字，突出核心要点

### 内容不重复
- 同一道题的各字段之间不要出现相同或高度相似的内容

## 输出前必须自我检查
在生成最终 JSON 之前，请逐条确认：
1. 检查 JSON 语法：所有键名和字符串值外有半角双引号，内容中的双引号已转义
2. 题目是否覆盖了内容中的核心知识点？是否遗漏重要内容？
3. 解析是否简洁有力，删除了废话？
4. 输出只有 \`\`\`json 代码块，无任何额外文字`;
...
## 内容质量
### 提炼与删减原则
- 删除背景介绍、历史渊源、冗长的修辞、重复的解释、次要的修饰词
- 长段落改为短句，删除"我们可以看到""需要注意的是"等废话
...
- 支持 HTML 标签排版：<b>加粗</b>、<i>斜体</i>、<br>换行、<ul>/<li>列表、<table>表格等
- 每道题可挖 1~3 个空，挖空内容应是理解该知识点必不可少的关键术语
- 如果原始题目已有下划线或括号标注的填空位置，按原始位置处理

示例：
\`"ClozeText": "<b>麻黄汤</b>的组成：<br>• [[麻黄]]<br>• [[桂枝]]<br>• [[杏仁]]<br>• [[甘草]]"\`
