...
function initCardMaker() {
    ...
    // Restore DeepSeek settings
    try {
        const dsKey = localStorage.getItem('kikkua_ds_key');
        const dsModel = localStorage.getItem('kikkua_ds_model');
        const keyEl = root.querySelector('#cmDsKey');
        const modelEl = root.querySelector('#cmDsModel');
        if (keyEl && dsKey) keyEl.value = dsKey;
        if (modelEl && dsModel) {
            for (const opt of modelEl.options) { if (opt.value === dsModel) { modelEl.value = dsModel; break; } }
        }
    } catch {}
